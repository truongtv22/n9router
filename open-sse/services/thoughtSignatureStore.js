import { makeKv } from "../../src/lib/kvStore.js";
...
/**
 * Store a thought signature for a tool_call_id with optional sessionId namespace (RAM + persisted kv async)
 */
export function storeGeminiThoughtSignature(toolCallId, signature, sessionId = null) {
  ...
  for (const k of keys) {
    ...
    // Async persist to the kv store without blocking
    signatureKv.set(k, {
      signature,
      createdAt: now,
...
/**
 * Retrieve a thought signature by tool_call_id (RAM first, then persisted kv fallback)
 */
